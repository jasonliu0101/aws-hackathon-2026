      ...
      <div className="mx-auto grid max-w-[1380px] items-start gap-4 xl:grid-cols-[430px_minmax(0,1fr)]">
        <aside className="space-y-4 xl:sticky xl:top-5">
          <section className="relative min-h-[315px] overflow-hidden rounded-[26px] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-blue-100 p-6 shadow-sm">
            ...
            <div className="absolute bottom-2 right-2 h-44 w-44"><Image src={asset("/assets/icons/ai-robot.webp")} alt="AI 助教" fill priority sizes="176px" className="object-contain drop-shadow-[0_18px_16px_rgba(79,70,229,.2)]" /></div>
          </section>
          ...
          <section className="rounded-[24px] border border-white/80 bg-white/95 p-5 shadow-sm">
            ...
            <div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-amber-50 p-4"><Image src={asset("/assets/badges/level-badge.webp")} alt="經驗值" width={34} height={34} className="h-9 w-9 object-contain" /><p className="mt-2 text-[11px] font-bold text-slate-500">已獲得經驗值</p><p className="text-lg font-black text-amber-700">{earnedXp} / {totalXp} XP</p></div><div className="rounded-2xl bg-yellow-50 p-4"><Image src={asset("/assets/badges/star-coin.webp")} alt="金幣" width={34} height={34} className="h-9 w-9 object-contain" /><p className="mt-2 text-[11px] font-bold text-slate-500">已獲得金幣</p><p className="text-lg font-black text-yellow-700">{earnedCoins} / {totalCoins}</p></div></div>
          </section>
            ...
            {displayTasks.map((task, index) => {
              ...
              return <article key={task.id} className={`relative overflow-hidden rounded-[22px] border p-4 transition sm:p-5 ${task.done ? "border-emerald-100 bg-emerald-50/50" : "border-slate-200 bg-white hover:-translate-y-0.5 hover:border-indigo-200 hover:shadow-md"}`}>
                <div className="grid items-center gap-4 sm:grid-cols-[64px_minmax(0,1fr)_180px]">
                  <div className="relative"><span className={`grid h-14 w-14 place-items-center rounded-2xl ${task.done ? "bg-emerald-100 text-emerald-600" : meta.tone}`}>{task.done ? <Check className="h-7 w-7" /> : <Icon className="h-7 w-7" />}</span><span className="absolute -left-1 -top-1 grid h-5 w-5 place-items-center rounded-full bg-slate-900 text-[9px] font-black text-white">{index + 1}</span></div>
